...
import uk.org.blankaspect.gui.TextArea;
...
////////////////////////////////////////////////////////////////////////
    ...
    private static final    String  SAVE_MESSAGE_STR        = "The search parameters have changed.\n" +
                                                                "Do you want to save the current search " +
                                                                "parameters?";
...
////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        private ScrollPane( TextArea textArea )
        {
            ...
            setBackground( AppConfig.getInstance( ).getTextAreaBackgroundColour( ) );
            setCorner( JScrollPane.LOWER_RIGHT_CORNER, new JPanel( ) );
            GuiUtilities.setViewportBorder( this, VERTICAL_MARGIN, HORIZONTAL_MARGIN );
            getViewport( ).setBackground( AppConfig.getInstance( ).getTextAreaBackgroundColour( ) );
...
////////////////////////////////////////////////////////////////////////
    ...
    public MainWindow( )
    {
        ...
        textView = new TextArea( config.getTextViewViewableSize( ).width,
        ...
        // Text area: results
        resultArea = new ResultArea( config.getTextViewViewableSize( ).width,
                                     config.getResultAreaNumRows( ) );
...
////////////////////////////////////////////////////////////////////////
        ...
        throws AppException
    {
        ...
        String command = AppConfig.getInstance( ).getEditorCommand( );
...
////////////////////////////////////////////////////////////////////////
                               ...
                               boolean      visible )
    {
        ...
        if ( visible )
        {
            ...
            currentPathname = (file == null)
                                ...
                                : Util.getPathname( file, AppConfig.getInstance( ).isShowUnixPathnames( ) );
            updateTitle( );
        }
        else
        {
    ...
    public void updateCommands( )
    {
        ...
        boolean canEdit = (textModel != null) && (textModel.getFile( ) != null) &&
                          (currentPathname != null) &&
                          (AppConfig.getInstance( ).getEditorCommand( ) != null);
    ...
    {
        setTitle( (currentPathname == null) ? App.LONG_NAME + " " + App.getInstance( ).getVersionString( )
    ...
    {
        int y = Math.max( 0, getResultList( ).getNumLines( ) - resultArea.getRows( ) ) *
                                                                                resultArea.getRowHeight( );
    ...
    {
        searchDialog = SearchDialog.showDialog( controlDialog, dialogKind,
                                                controlDialog.getTargetString( true ), currentPathname,
                                                textModel.getSelection( ).startRow,
                                                textModel.getSelectionStart( ),
                                                textModel.getSelectionEnd( ) );
    }
    ...
    private File chooseSave( File file )
    {
        ...
        saveFileChooser.setSelectedFile( (file == null) ? new File( new String( ) ) : file );
        saveFileChooser.rescanCurrentDirectory( );
        return ( (saveFileChooser.showSaveDialog( this ) == JFileChooser.APPROVE_OPTION)
                                                    ? Util.appendSuffix( saveFileChooser.getSelectedFile( ),
                                                                         AppConstants.XML_FILE_SUFFIX )
                                                    : null );
    }
    ...
    {
        // Perform search
        TextSearcher textSearcher = App.getInstance( ).getTextSearcher( );
        try
        {
            // Hide control dialog
            if ( controlDialog.isVisible( ) &&
                 AppConfig.getInstance( ).isHideControlDialogWhenSearching( ) )
            {
        ...
        if ( searchDialog == null )
        {
            ...
            getResultList( ).addAggregate( textSearcher.getAggregateResult( ) );

            // Update results with errors, unprocessed files and files whose attributes were not set
            List<File> files = PathnameFilter.getErrors( );
            if ( !files.isEmpty( ) )
                getResultList( ).addFiles( UNPROCESSED_STR + ": " + NO_CANONICAL_PATHNAME_STR, files,
                                           false );
            files = textSearcher.getUnprocessedFiles( );
            if ( !files.isEmpty( ) )
                getResultList( ).addFiles( UNPROCESSED_STR + ": " + PROCESSING_ERROR_STR, files, true );
            files = textSearcher.getAttributesNotSetFiles( );
            if ( !files.isEmpty( ) )
                getResultList( ).addFiles( UNPROCESSED_STR + ": " + ATTRIBUTES_NOT_SET_STR, files, true );
    ...
    private void updateConfiguration( )
    {
        ...
        if ( !controlDialog.getInitialParameterEditorSize( ).equals( size ) )
        {
            ...
            size.height = Math.min( Math.max( ParameterEditor.MIN_NUM_ROWS, size.height ),
        ...
        throws AppException
    {
        ...
        // Open file
        if ( file != null )
            App.getInstance( ).openSearchParams( file );
    ...
    private void onCopyResults( )
        throws AppException
    {
        String text = AppConfig.getInstance( ).isCopyResultsAsListFile( )
                                            ? getResultList( ).getText( ControlDialog.COMMENT_PREFIX_CHAR )
                                            : getResultList( ).getText( );
        Util.putClipboardText( text );
    }
    ...
    private void onViewSavedResults( )
    {
        ...
        for ( int i = 0; i < files.size( ); ++i )
        {
            if ( i > 0 )
                buffer.append( '\n' );
            buffer.append( Util.getPathname( files.get( i ) ) );
        }
